module.exports.run = (client, message, args) => {
  const role = message.guild.roles.cache.find(r => r.name === 'Beta Tester');
  if (!role) return message.reply(':x: I did not find the `Beta Tester` role.');
  if (!message.member.roles.cache.has(role.id)) return message.reply(':x: You required the `Beta Tester` role to execute this command.');

  if (!args[1]) return message.reply(':x: You must provide something to suggest.');
  const info = args.slice(1).join(' ');

  const channel = message.guild.channels.cache.find(c => c.type === 'text' && c.name === 'beta-tester-bug-reports');
